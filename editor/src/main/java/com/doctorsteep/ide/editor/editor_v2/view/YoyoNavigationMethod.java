...
import com.doctorsteep.ide.editor.editor_v2.view.ColorScheme.*;
...
public class YoyoNavigationMethod extends TouchNavigationMethod {
	...
	public YoyoNavigationMethod(FreeScrollingTextField textField) {
		...
		_yoyoSize = (int) TypedValue.applyDimension(2, (float)(textField.BASE_TEXT_SIZE_PIXELS * 1.4), dm);
		_yoyoCaret = new Yoyo();
		_yoyoStart = new Yoyo();
		_yoyoEnd = new Yoyo();
	}
	...
	private class Yoyo {
		...
		public void draw(Canvas canvas, boolean activated) {
			...
			canvas.drawArc(new RectF(_anchorX - radius, _anchorY - radius / 2 - YOYO_STRING_RESTING_HEIGHT,
									 _handleX + radius * 2, _handleY + radius / 2), 51, 78, true, _brush);
